...
(ert-deftest douban-browser-cookie-reader-delegates-to-browser-cookies ()
  (let ((douban-cookie-browser 'firefox)
        (douban-cookie-profile-directory "/profiles/douban/")
        (douban-firefox-origin-attributes "^userContextId=2")
        called)
    (cl-letf (((symbol-function 'browser-cookies-get)
               (lambda (url &rest arguments)
                 (setq called (cons url arguments))
                 '(("dbcl2" . "session")))))
      (should
       (equal
        (douban--read-browser-cookies "https://www.douban.com/mine/")
        '(("dbcl2" . "session")))))
    (should
     (equal called
            '("https://www.douban.com/mine/"
              :browser firefox
              :profile-directory "/profiles/douban/"
